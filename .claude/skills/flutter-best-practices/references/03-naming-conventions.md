...
| 全局通用组件 | `Sky` 前缀 + 语义名词 | `SkyButton`, `SkyCheckbox` |
...
| 场景 | 约定 | 示例 |
| --- | --- | --- |
| 全局复用 UI 组件 | 类名加 `Sky` 前缀；文件 `sky_*.dart`；放 `lib/shared/widgets/<组件类别>/` 子目录 | `SkyButton`（`widgets/button/sky_button.dart`）、`SkyCheckbox`、`SkyCheckboxFormField` |
| 全局主题令牌类 | `Sky<角色>`（不带 App） | `SkyColors`、`SkyTheme`、`SkyFonts`、`SkyTextStyles`、`SkySpacing`、`SkySemanticSpacing`、`SkyRadius`、`SkyBorderWidth`、`SkyIconSize`、`SkyControlSize`、`SkyGradients`、`SkyShadows`、`SkyMotion` |
| feature 私有组件 | **不加** `Sky`；语义 `UpperCamelCase`；文件 `snake_case.dart`；放该 feature 的 `widgets/` | `AuthInput`（`features/auth/widgets/auth_input.dart`）、`SocialIconButton` |
| 布尔入参 | `is` 前缀 | `isLoading`、`isFullWidth`、`isError` |
| 事件回调入参 | `on` 前缀 | `onPressed`、`onChanged`、`onTap` |

- `Sky` 前缀 = “**全局可复用**”的标志：只有下沉到 `lib/shared/widgets/` 的通用组件才用；feature 内部组件不要用，避免误当作可跨 feature 复用。
