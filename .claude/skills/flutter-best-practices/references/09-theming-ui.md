...
## 📌 AI 状态表达（`lib/shared/widgets/ai/`）

流式生成的每个阶段都必须有**明确不同**的视觉表达，否则用户分不清"在思考""在写""写完了""挂了"，进而重复点击、误以为卡死。

| 组件 | 阶段 | 要点 |
| --- | --- | --- |
| `HappyThinkingIndicator` | 请求已发出、首字未到 | 这段窗口可能长达数秒，**必须**占位。三点波浪 + 流光文案 |
| `HappyStreamingText` | 正在写 | 末尾呼吸光标是"AI 正在写"唯一的视觉证据。用柔和淡入而非编辑器式硬闪 |
| `HappyMarkdownText` | 输出带结构 | 基于 `gpt_markdown`，能容忍流式中**未闭合**的语法。散文正文用 `HappyStreamingText`，别用它 |
| `HappyShimmerText` | 任意"系统在忙" | 每帧重建 shader，**只给一行状态文案**，别用在长段落 |
| `HappyRetryCard` | 失败 | **内联**在内容下方，不是弹窗、不是整页错误态。上方已生成的文字必须留在原地 |

三条硬约束：

1. **失败不清屏**。流式失败常常是已经生成了一半，用户等来的半篇不能因为一次网络抖动被清掉。状态机用 `StoryGenerationState.failed(text:, failure:)` 携带残篇，别用 `AsyncError`（它丢 data）。
2. **思考态和生成态是两种等待**，不能都用 loading 表达（这也是这套状态机不用 `AsyncValue` 的原因）。
3. **自动滚动但不抢滚动条**。内容增长时贴底滚动，但用户手动往上翻看前文时不要把他拽回底部——判定"是否贴底"要在**状态更新之前**做，此时 `position` 还是旧布局的，才反映用户看新内容前的真实意图。参考 `StoryScreen._autoScrollIfPinned()`。

对应的数据侧规范（SSE 解码、匀速吐字、停止生成）见 `05-networking.md`。
...
- **流式生成不用骨架屏**：骨架屏表达"结构已知、数据未到"，而流式是内容逐字长出来的。用 `HappyThinkingIndicator` → `HappyStreamingText`。
