...
class _HappyOtpFieldState extends State<HappyOtpField> {
      ...
      builder: (field) {
        ...
        return Column(
          ...
          children: <Widget>[
            ...
            if (field.hasError)
              Padding(
                padding: const EdgeInsets.only(top: HappySpacing.s4),
                child: Text(
                  field.errorText!,
  ...
  Widget _buildBox(BuildContext context, int index, bool hasError) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: borderColor,
          width: isActive ? HappyBorderWidth.thick : HappyBorderWidth.hairline,
        ),
        // 深色画布上只靠描边不够醒目，活动格与成功态补一层品牌光晕。
        boxShadow: isSuccess || (isActive && !hasError)
            ? HappyShadows.glow(scheme.primary, intensity: 0.2)
            : HappyShadows.none,
      ),
      child: isSuccess
  ...
  Widget _buildCheck(BuildContext context, int index) {
    ...
    return check
        ...
        .fadeIn(duration: HappyMotion.fast);
  }
  ...
  Widget _buildDigit(BuildContext context, String digit, int index) {
    ...
    return text
        ...
        .scaleXY(
          ...
          delay: HappyMotion.stagger * (index - _fillStart),
        );
  }
...
class _Caret extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return bar
        ...
        .fadeOut(duration: HappyMotion.slow, curve: HappyMotion.ambientCurve);
  }
}
